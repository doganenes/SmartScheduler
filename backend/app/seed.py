...
def seed(clear_db=True):
    ...
    # 3. Classes (8 Classes: 9-A to 12-B)
    class_names = []
    for g in ["9", "10", "11", "12"]:
        for s in ["A", "B"]:
            class_names.append(f"{g}-{s}")
    ...
    if teachers and subjects and classes:
        import random
        print("Assigning subjects to teachers across ALL 8 classes...")
        
        for teacher in teachers:
            # Pick 1 random subject for this teacher to keep workload manageable across 8 classes
            # (8 classes * 2 hours = 16 hours/week)
            teacher_subjects = random.sample(subjects, 1)
            
            for sub in teacher_subjects:
                # Assign this subject to THIS teacher for ALL 8 classes
                for cls in classes:
        ...
        db.commit()
        print("Courses seeded across all 8 classes for each teacher assignment.")
